...
layui.use(['form','jquery',"layer"],function() {
    ...
    function showNotice(){
        layer.open({
            ...
            btn: ['火速围观'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">请使用本网站后台管理v2.0系统前请务必仔细阅读首页右下角的《使用文档说明》，避免操作使用中遇到一些简单的问题造成困扰。</div>',
    ...
    //锁屏
    //查询当前用户是否锁屏
    $.get('/api/admin/index/check-lock', function (res) {
        if (res) {
    ...
    $(".lockcms").on("click",function(){
        ...
        lockPage();
        //发送锁屏信号
        $.get('/api/admin/index/lock', function (res) {
        });
    });
    // 判断是否显示锁屏
    ...
    $("body").on("click","#unlock",function(){
        if($(this).siblings(".admin-header-lock-input").val() == ''){
            ...
            // if($(this).siblings(".admin-header-lock-input").val() == "111111"){
            ...
            // }
            var lockString = $(this).siblings(".admin-header-lock-input").val();
            $.post('/api/admin/index/clear-lock', {pass: lockString}, function (res) {
                window.sessionStorage.setItem("lockcms", false);
                $(this).siblings(".admin-header-lock-input").val('');
                layer.closeAll("page");
            }).error(function (msg) {
                layer.msg(eval(msg.responseText)[0].message);
                $(this).siblings(".admin-header-lock-input").val('').focus();
            });
        }
    });
