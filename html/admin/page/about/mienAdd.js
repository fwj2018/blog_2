layui.use(['form','layer','layedit','laydate','upload'],function(){
    ...
    upload.render({
        elem: '.thumbBox',
        url: DIR +'/api/admin/mien/upload-image',
    ...
    form.on("submit(mienAdd)",function(data){
        //截取文章内容中的一部分文字放入文章摘要
        // var abstract = layedit.getText(editIndex).substring(0,50);
        //弹出loading
        // var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
        //实际使用时的提交信息
        $.post(DIR + "/api/admin/mien/mien-edit",{
            id: $('#id').val(),
            name : $(".name").val(),  //文章标题
            ...
            $('#id').val() != '' ? top.layer.msg("风采编辑成功！") : top.layer.msg("风采添加成功！");
    ...
    var editIndex = layedit.build('news_content',{
        height : 535,
        uploadImage: {
            url: '/api/admin/mien/upload-image', //接口url
            type: 'post' //默认post
        }
    });
